...
const renderCurrentWeatherCard = function (currentData) {
  const currentWeatherCard = `<div class="card-body bg-dark border mb-2">
    <h2 class="card-title">
        ${currentData.name} ${currentData.date}
        <span class="bg-transparent m-2"><img src="https://openweathermap.org/img/w/${
    ...
    <p class="card-text">
        UV index: <span class="bg-light p-2 ${getUVIClassName(currentData.uvi)}">${
    currentData.uvi
  }</span>
    </p>
    </div>`;
...
const renderForecastWeatherCards = function (forecastData) {
  const constructForecastCard = function (each) {
    return `<div class="card m-1 forecast-card">
        <div class="card-body bg-dark bg-opacity-25 text-dark">
        <h5 class="card-title text-center">${each.date}</h5>
  ...
  const forecastCardsContainer = `<div class="bg-dark border">
